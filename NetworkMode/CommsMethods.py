...
def wait_for_desired_message(comms_info, desired_message, timeout):
    """
    Waits until it receives the message matching the desired value on the given pipe, or until the timeout_ms expires.
    
    Parameters:
    - listening_pipe_adress: The pipe being listened to for incoming messages.
    - responder_pipe_adress: The pipe where the response should be sent.
    ...
    """
    nrf.open_rx_pipe(1, comms_info.listening_pipe_adress)
    ...
    while (time.monotonic() - start) < timeout:
        if nrf.available():
            ...
            comms_info.responder_pipe_adress = received_message[:5]
