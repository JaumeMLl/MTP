...
def wait_for_desired_message(comms_info, desired_message, timeout, nrf):
    ...
    while (time.monotonic() - start) < timeout:
        if nrf.available():
            ...
            if actual_message == desired_message:
                logging.info(f"Desired Message {desired_message} received")
    ...
    logging.info(f"Desired Message {desired_message} NOT received")
    return False
...
def send_message(destination_pipe_address, message, nrf):
    ...
    logging.info(f"About to send: {data_to_send}", "TO:", destination_pipe_address)
    ...
    if sent_successfully:
        ...
        logging.info(f"Failed to send {message}.")
        return False
...
def transmitter(comms_info, filelist, count, nrf):
    ...
    for i in range(total_chunks):
        ...
        logging.info(result)
        logging.info((i+1)/total_chunks, " %")
        r = True

    if r:
        logging.info("Package Transmission Accomplished")
    else:
        logging.info("Package Transmission Failed")
    return r 
...
def receiver(comms_info, timeout, nrf):
    ...
    while (time.monotonic() - start) < timeout:
        if nrf.available():
            while nrf.available():
                ...
                logging.info("BIT RECEIVED:", received_bit_flip, "EXPECTED BIT: ", rx_bit_flip)
                if received_bit_flip == rx_bit_flip or not stop_wait:
                    logging.info("Received chunk: {}...".format(buffer[:10]))
                    received_data += buffer[1:]  
                    rx_bit_flip ^= 1
                else :
                    logging.info("Out of sequence packet")
            start = time.monotonic()
    nrf.listen = False
    ...
    with open(FILE_NAME, "wb") as file:
        ...
        logging.info(f"Archivo reconstruido y guardado. Tamaño total: {len(received_data)} bytes.")
    #nrf.auto_ack=False
    ...
    try:
        for txt_file in txt_files:
            ...
            logging.info(f"Received message '{txt_file}' also stored in '/media/usb/'")
            #newfile_leds()
            #blink_success_leds(10, USB_LED, USB_LED) 
            #reset_leds()
    except Exception as e:
        logging.info(f"Failed to save the message in '/media/usb'. Error: {e}")
